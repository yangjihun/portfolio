...
export const projects: Project[] = [
  {
    ...
    troubleshooting: [
      ...
      {
        title: '질문과 관련된 구간을 찾아 챗봇 답변 정확도와 요청 크기 개선',
        problem:
        '3시간 수업의 전체 전사는 약 180KB로 요청 크기 제한을 넘을 수 있었습니다. 이를 피하기 위해 드래그한 구간과 앞뒤 구간의 전사만 참고했지만, 구간 자체가 길면 여전히 요청 예산을 초과할 위험이 존재했고 선택 지점에서 멀리 떨어진 내용을 묻는 질문에는 답하지 못했습니다.',
        solution:
        '전체 목차와 구간별 요약에서 질문과 관련된 구간을 최대 3개 추가 선별한 뒤, 선택 지점 주변 전사와 함께 답변에 활용하는 2단계 검색 구조를 도입했습니다. 선택 지점 주변의 전사 범위에는 30분 상한을 두고, 완성된 요청이 예산을 초과하면 대화 이력과 구간 요약 순으로 줄이도록 했습니다. 인용문은 모델이 생성한 문장 대신 서버에 저장된 실제 전사에서 가져오고, 전달한 전사 범위에 속하는지 검증했습니다.',
        result:
        '합성 전사 평가에서 앵커 밖 질문 정답률을 87.5%로 높였습니다. 또한 선택 지점 주변 전사에 30분 상한을 두어 요청 용량을 줄이고, 최종 본문 크기를 검사해 예산을 초과한 요청의 전송을 방지했습니다.'
      },
    ],
    links: []
  },
